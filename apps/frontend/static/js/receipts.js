    ...
    return `<tr>
      ...
      <td>
        <div style="display:flex;gap:6px;">
          <a href="/receipts/${r.id}/edit/" class="btn btn-outline btn-sm btn-icon" title="Edit">✏️</a>
          <button class="btn btn-danger btn-sm btn-icon" title="Delete" onclick="openDeleteModal(${r.id})">🗑️</button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
function getStatusBadge(days) {
  ...
  if (days <= 7)  return `<span class="badge badge-red">⚠️ ${days}d left</span>`;
